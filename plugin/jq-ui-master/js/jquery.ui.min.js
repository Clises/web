(function ($) {
    function initPanel($panel, options, content) {
        ...
        var $header = $("<div></div>").addClass("panel-header").append($("<div></div>").addClass("panel-title").html(options["title"]));
        var $content = $("<div></div>").addClass("panel-content").html(content);
    ...
    function initSize($panel, options) {
        ...
        setTimeout(function () {
            $panel.find(".panel-content").css({"width": ($panel.width() - 20), "height": ($panel.height() - 46)})
        }, 10)
    }
    ...
    function _load($panel, url) {
        ...
        $.ajax({
            type: "post", dataType: "html", url: url, cache: false, success: function (data) {
    ...
    $.fn.panel = function (options, param) {
        ...
        var defaults = {title: "", width: "0", height: "0", url: ""};
...
(function ($) {
    function createButton(text, fn) {
        return $("<button>" + text + "</button>").addClass("default-button").click(function () {
    ...
    $.messager = {
        __top: 3, alert: function (options) {
            var defaultSettings = {
                title: "信息",
                width: 250,
                height: 170,
            ...
            var messageContent = $("<div></div>").addClass("message-content").append($("<span></span>").addClass("message-" + options["level"] + "-icon message-icon")).append($("<span>" + options["content"] + "</span>"));
            ...
            var options = $.extend(defaultSettings, options);
            var messageContent = $("<div></div>").addClass("message-content").append($("<span></span>").addClass("message-question-icon message-icon")).append($("<span>" + options["content"] + "</span>"));
            var messageButtonContainer = $("<div></div>").addClass("message-button-container");
            ...
            var options = $.extend(defaultSettings, options);
            var messageContent = $("<div></div>").addClass("message-content").css("text-align", "center").append($("<span>" + options["content"] + "</span>"));
...
(function ($) {
    ...
    function initDialog(id, title) {
        var $dialog = $("<div></div>").attr("id", id).addClass("dialog-container");
        var $header = $("<div></div>").addClass("dialog-header").append($("<div>" + title + "</div>").addClass("dialog-title")).append($("<div></div>").addClass("dialog-close"));
        var $content = $("<div></div>").addClass("dialog-content");
    ...
    function initSize(dialog, options) {
        ...
        dialog.find("div.dialog-content").css({"width": options["width"] - 32, "height": options["height"] - 57})
    }
    ...
    function bindDrag(dialog) {
        ...
        $(document).mousemove(function (event) {
            ...
            var y = event.screenY;
            if (isDown) {
                dialog.css({"left": x - mX + dX, "top": y - mY + dY})
            }
        });
...
(function ($) {
    function initTable($datatable) {
        ...
        $("<thead></thead>").append(tr).appendTo($datatable.addClass("datatable").attr({
            "cellpadding": "0",
            "cellspacing": "0"
        }));
...
(function ($) {
    function init($pagination) {
        $pagination.addClass("pagination").append($('<a href="javascript:;" class="first">首页</a>')).append($('<a href="javascript:;" class="prev">上一页</a>')).append($('<div class="page-container"></div>')).append($('<a href="javascript:;" class="next">下一页</a>')).append($('<a href="javascript:;" class="last">尾页</a>'));
    ...
    $.fn.pagination = function (options, param) {
        ...
        var defaults = {
            pageNum: 1, size: 10, total: 1, click: function () {
                return 1
            }
        };
        var options = $.extend(defaults, options);
...
(function ($) {
    function init($accordion) {
        $accordion.addClass("accordion").children("li:even").addClass("accordion-title").end().children("li:odd").addClass("accordion-body").css("display", "none").end().children("li.accordion-title").filter(".active").next().slideDown("100")
    }
...
(function ($) {
    function initTab($tab) {
        $tab.addClass("tab").children("ul").addClass("tab-header").append($("<li></li>").addClass("tab-header-items").css("width", $tab.outerWidth() - 25).append("<ul></ul>")).append($("<li></li>").addClass("tab-header-select tab-header-select-down").css("title", "显示隐藏").append("<ul class='hide-title-list'></ul>"));
    ...
    function initEvents($tab) {
        $tab.delegate(".tab-header-item", "click", function () {
    ...
    function initContextMenu() {
        ...
        if (!contextMenu[0]) {
            $("<div class='tab-contextmenu'></div>").append(createContextMenuItem("关闭当前标签", "current")).append(createContextMenuItem("关闭左侧标签", "prevAll")).append(createContextMenuItem("关闭右侧标签", "nextAll")).append(createContextMenuItem("关闭其他", "other")).append(createContextMenuItem("关闭全部", "all")).appendTo("body")
        }
    ...
    function initWindowContextMenu() {
        $("body").bind("contextmenu", contextMenuHandler).on("click", function (ev) {
    ...
    function addTab($tab, param) {
        if (isExists($tab, param["id"])) {
            ...
            $newHeaderItem = $("<li></li>");
            $newHeaderItem.text(param["title"]).attr("target", param["id"]).addClass("tab-header-item").addClass("tab-header-selected").append($('<span class="close"></span>')).appendTo($tab.find(".tab-header-items").children("ul"));
            $tab.children("div").children().addClass("hide");
            $newContentItem = $("<div></div>");
            $newContentItem.html(param["content"]).attr("id", param["id"]).addClass("tab-content-item").appendTo($tab.children("div"));
            afterAddTab($tab)
        }
    }
    ...
    function afterAddTab($tab) {
        ...
        for (var j = maxVisible; j < titles.length - 1; j++) {
            var t = titles.eq(j);
            t.removeClass("tab-header-selected").attr("real-width", t.outerWidth()).appendTo($tab.find(".tab-overflow-items"));
    ...
    function closeSelect() {
        if ($(".tab-header-select").hasClass("tab-header-select-up")) {
            $(".tab-header-select").removeClass("tab-header-select-up").addClass("tab-header-select-down").children().css("display", "none")
        }
    }
    ...
    function removeTab($tab, tabId) {
        ...
        if (prevItem[0]) {
            ...
            if (hide1[0]) {
                nextSelectedId = hide1.attr("item-id")
            }
        }
        headerItem.remove();
    ...
    function selectTab($tab, tabId) {
        ...
        console.log(tab);
        if (tab[0]) {
            tab.addClass("tab-header-selected").siblings().removeClass("tab-header-selected")
        } else {
        ...
        $tab.find("#" + tabId).removeClass("hide").siblings().addClass("hide");
        afterAddTab($tab)
    }
...
(function ($) {
    function _initTree($tree) {
        $tree.find("li[class!='node'][class!='folder']").addClass("node").end().find("li:has('ul')").removeClass("node").addClass("folder");
    ...
    function _createNodes($target, data, options) {
        ...
        for (var i = 0; i < data.length; i++) {
            var d = data[i];
            var $node = $("<li></li>").addClass("node").attr("node-id", "tree-node-" + d["id"]);
            var $a = $("<a></a>").html(d["text"]).prepend("<span><span>");
            if (d["url"]) {
                $a.attr("href", d["url"])
            } else {
                if (options["onClick"] && !d["type"]) {
