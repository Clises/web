...
//          $str .= "<a href='{$this->uri}page=".($this->page-1)."'>{$this->config["prev"]}</a> ";
            ...
            $str = " <a href='javascript:showpage(\"{$this->uri}page=1\")'>{$this->config["first"]}</a> ";
            $str .= "<a href='javascript:showpage(\"{$this->uri}page=".($this->page-1)."\")'>{$this->config["prev"]}</a> ";
            return $str;
        }
...
//                $linkPage .= "<a href='{$this->uri}page={$page}'>{$page}</a> ";
    ...
    private function nextlast(){
        if($this->page != $this->pageNum) {
            $str = " <a href='javascript:showpage(\"{$this->uri}page=".($this->page+1)."\")'>{$this->config["next"]}</a> ";
            $str .= " <a href='javascript:showpage(\"{$this->uri}page=".($this->pageNum)."\")'>{$this->config["last"]}</a> ";
            return $str;
        }
    }
    ...
    private function goPage(){
        if($this->pageNum > 1) {
            return ' <input style="width:20px;height:17px !important;height:18px;border:1px solid #CCCCCC;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>'.$this->pageNum.')?'.$this->pageNum.':this.value;showpage(\''.$this->uri.'page=\'+page+\'\')}" value="'.$this->page.'"><input style="cursor:pointer;width:25px;height:18px;border:1px solid #CCCCCC;" type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>'.$this->pageNum.')?'.$this->pageNum.':this.previousSibling.value;showpage(\''.$this->uri.'page=\'+page+\'\')"> ';
        }
    }
