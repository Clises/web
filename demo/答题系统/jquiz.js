(function($) {
    $.fn.jquizzy = function(settings) {
        ...
        answers = [],
        introFob = '	<div class="intro-container slide-container"><a class="nav-start" href="#">请按照您的实际情况依次答题,准备好了吗？<br/><br/><span><img src="'+config.startImg+'"></span></a></div>	',
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
            ...
            for (answersIteratorIndex = 0; answersIteratorIndex < config.questions[questionsIteratorIndex].answers.length; answersIteratorIndex++) {
                contentFob += '<li>' + config.questions[questionsIteratorIndex].answers[answersIteratorIndex].answers + '</li>';
            }
        ...
        function countScore(){
            ...
            for (i = 0; i < config.questions.length; i++) {
                j = userAnswers[i]-1;
                var answerInfo = config.questions[i].answers[j];
                console.log(answerInfo);
                score += Number(answerInfo.point);

                if(answerInfo.feel_info != ''){
                    feel_info.push(answerInfo.feel_info);
                }

                if(answerInfo.honor_info != ''){
                    honor_info.push(answerInfo.honor_info);
                }

                if(answerInfo.money_info != ''){
                    money_info.push(answerInfo.money_info);
                }

                if(answerInfo.risk_info != ''){
                    risk_info.push(answerInfo.risk_info);
                }

                if(answerInfo.time_info != ''){
                    time_info.push(answerInfo.time_info);
                }

            }
            var data = {'score':score,'feel_info':feel_info,'honor_info':honor_info,'money_info':money_info,'risk_info':risk_info,'time_info':time_info}
            return data;
        }
        ...
        superContainer.find('.final').click(function() {
            ...
            trueCount = 0,
            shareButton = '',
            answerResults = countScore(),
            score = answerResults.score,
            url;
            ...
            resultSet = '<h2 class="qTitle">' + score + '<br/> 您的分数： ' + score + '</h2>' + shareButton + '<div class="jquizzy-clear"></div>' + resultSet + '<div class="jquizzy-clear"></div>';
